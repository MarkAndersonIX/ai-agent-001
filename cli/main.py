...
class AgentCLI:
    ...
    def _get_session_id(self, agent_type: str, session_name: Optional[str] = None) -> str:
    ...
    def _make_request(self, method: str, endpoint: str, **kwargs) -> requests.Response:
        ...
        try:
            ...
            click.echo(f"Error: Could not connect to API server at {self.base_url}", err=True)
            click.echo("Make sure the server is running with: python -m api.server", err=True)
            sys.exit(1)
            ...
            if show_sources and data.get('sources'):
                ...
                for i, source in enumerate(data['sources'], 1):
                    if 'url' in source:
                        ...
                        click.echo(f"  {i}. {source.get('metadata', {}).get('title', 'Document')}")
    ...
    def show_session_history(self, agent_type: str, session_name: Optional[str] = None) -> None:
        """Show session conversation history."""
        session_id = self._get_session_id(agent_type, session_name)
        
        response = self._make_request('GET', f'/agents/{agent_type}/sessions/{session_id}')
    ...
    def execute_tool(self, agent_type: str, tool_name: str, input_text: str) -> None:
        ...
        if response.status_code == 200:
            ...
            if data.get('success'):
                ...
                click.echo(f"Tool execution failed: {data.get('error', 'Unknown error')}", err=True)
        else:
            click.echo(f"Error: {response.status_code} - {response.text}", err=True)
...
@cli.command()
@click.argument('agent_type', type=click.Choice(['general', 'code_assistant', 'research_agent', 'document_qa']))
