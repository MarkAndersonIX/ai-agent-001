...
class CalculatorTool(BaseTool):
    ...
    def _normalize_expression(self, input_text: str) -> str:
        ...
        expression = re.sub(r'^(calculate|compute|eval|evaluate)\s+', '', input_text.strip(), flags=re.IGNORECASE)
        ...
        for func, replacement in math_functions.items():
            ...
            for match in matches:
                expression = expression.replace(f'{func}({match})', replacement.format(match))
