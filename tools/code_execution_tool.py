...
class CodeExecutionTool(BaseTool):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.allowed_languages = config.get('allowed_languages', ['python', 'javascript', 'bash'])
        ...
        self.language_configs = {
            'python': {
                'command': ['python3', '-c'],
                'file_extension': '.py',
                'unsafe_imports': ['os', 'subprocess', 'sys', 'socket', 'urllib', 'requests']
            },
            'javascript': {
                'command': ['node', '-e'],
                'file_extension': '.js',
                'unsafe_patterns': ['require("fs")', 'require("child_process")', 'require("net")']
            },
            'bash': {
                'command': ['bash', '-c'],
                'file_extension': '.sh',
                'unsafe_commands': ['rm', 'mv', 'cp', 'chmod', 'sudo', 'su', 'wget', 'curl']
            }
        }
    ...
    def _parse_code_input(self, input_text: str) -> tuple[Optional[str], Optional[str]]:
        ...
        if any(indicator in input_text for indicator in ['print(', 'def ', 'import ', 'for ', 'if ']):
            return 'python', input_text
        
        # Format 5: Assume JavaScript if looks like JS
        if any(indicator in input_text for indicator in ['console.log', 'function ', 'const ', 'let ', 'var ']):
