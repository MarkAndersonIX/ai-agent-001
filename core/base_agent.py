...
class BaseAgent(ABC):
    ...
    def _initialize_components(self) -> None:
        ...
        try:
            ...
            self.vector_store: VectorStore = ComponentFactory.create_vector_store(vector_config)
            
            # Document store
            doc_config = self.config.get_section("document_store")
            self.document_store: DocumentStore = ComponentFactory.create_document_store(doc_config)
            
            # Memory backend
            memory_config = self.config.get_section("memory")
            self.memory_backend: MemoryBackend = ComponentFactory.create_memory_backend(memory_config)
            
            # LLM provider
            llm_config = self.config.get_section("llm")
            self.llm_provider: LLMProvider = ComponentFactory.create_llm_provider(llm_config)
            
            # Embedding provider
            embedding_config = self.config.get_section("embedding")
            self.embedding_provider: EmbeddingProvider = ComponentFactory.create_embedding_provider(embedding_config)
        ...
        try:
            ...
            for result in relevant_results:
                ...
                if "source_url" in metadata:
                    ...
                    source_info["title"] = metadata.get("original_title", "Web Document")
        ...
        try:
            ...
            for msg in recent_history:
                if msg.role == "user":
                    ...
                    messages.append(self.llm_provider.create_assistant_message(msg.content))
    ...
    def _save_conversation_turn(
        ...
        try:
            ...
            self.memory_backend.append_message(session_id, assistant_msg, self.agent_type)
