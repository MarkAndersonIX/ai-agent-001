...
class ChromaVectorStore(VectorStore):
    ...
    def add_documents(self, documents: List[Document]) -> List[str]:
        ...
        try:
            ...
            if doc_embeddings and len(doc_embeddings) == len(documents):
                ...
                self.collection.add(
                    ids=doc_ids, documents=doc_contents, metadatas=doc_metadatas
                )
        ...
        try:
            ...
            if results and results["documents"] and results["documents"][0]:
                ...
                for i, content in enumerate(documents):
                    ...
                    similarity_score = (
                        1.0 - distance if distance <= 1.0 else 1.0 / (1.0 + distance)
                    )
    ...
    def list_documents(
        ...
        try:
            ...
            # Get documents
            kwargs = {"include": ["documents", "metadatas"], "where": where_clause}
            ...
            if results and results["documents"]:
                for i, content in enumerate(results["documents"]):
                    ...
                    doc_id = (
                        results["ids"][i]
                        if results["ids"] and i < len(results["ids"])
                        else None
                    )
    ...
    def count_documents(self, filters: Optional[Dict[str, Any]] = None) -> int:
        ...
        try:
            ...
            # Get count
            results = self.collection.get(
                where=where_clause, include=[]  # Don't include content, just count
            )
